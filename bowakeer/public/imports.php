...
<?php
	...
	$imports = import::find_by_sql($sql);
    $sql_all = "SELECT * FROM imports ";
    $all_imports = export::find_by_sql($sql_all);;
...
<header class="w3-container w3-theme" style="padding:21px 32px">
  ...
  <div class="w3-row">
    ...
    <div class="w3-bar">
   <a href="imports.php" class="w3-bar-item w3-button w3-round-medium">Home</a>
...
<div class="w3-container" style="padding:32px;">
    ...
    <div class="w3-modal-content w3-card-4 w3-animate-top" style="max-width:600px">
      ...
      <form class="w3-container" action="actions/add/import.php" method="POST">
        <div class="w3-section">
          <input class="w3-input w3-border" type="number" placeholder="Company Id" name="company_id" required>
          <input class="w3-input w3-border" type="date" placeholder="Import Date" name="import_date" required>
		  <input class="w3-input w3-border" type="text" placeholder="Payment Type" name="payment_type" required>
        </div>
    ...
    <div class="w3-modal-content w3-animate-zoom" style="max-width:100%">
      ...
      <div id="modal-data"  class="w3-col m12" style="">
        ...
        ?>
            ...
            <?php foreach($all_imports as $imp): ?>
            <tr>
                <td><?php echo $imp->id; ?></td>
                <td><?php echo $imp->company_id; ?></td>
                <td><?php echo $imp->import_date; ?></td>
				<td><?php echo $imp->payment_type; ?></td>
            </tr>
            <?php endforeach; ?>
        </table>
        </div>
    ...
    <div class="w3-right w3-col m4">
        ...
        <select class="w3-select w3-border" style="float: left; width: 29%;" name="search_term">
            ...
            <datalist id="cusom-list">
                <?php
                   $sql = "SELECT import_date FROM imports;";
                   $result = import::find_by_sql($sql);
                   //var_dump($result);
                   foreach($result as $op){
                    echo "<option value=\"{$op->import_date}\">" ;
                   }
                ?>
            </datalist>
    ...
    <div class="w3-col m12" style="">
        ...
        ?>
            ...
            <tr>
                ...
                <a href="edit_import.php?id=<?php echo $imp->id?>" class="w3-button w3-button-small w3-teal" title="Edit"><i class="fa fa-pencil"></i></a>    
                <a href="actions/delete/delete_import.php?id=<?php echo $imp->id?>" class="w3-button w3-button-small w3-red" onclick="return confirm('Are you sure you want to delete import: <?php echo $imp->id?>?')" title="Delete"><i class="fa fa-trash"></i></a>
                <a href="actions/print/print_import.php?id=<?php echo $imp->id?>" class="w3-button w3-button-small w3-blue" title="Print"><i class="fa fa-print"></i></a></td>
            </tr>
            <?php endforeach; ?>
        </table>
        </div>
      ...
      echo "\" class=\"w3-bar-item w3-button\">&laquo;</a> "; 
		...
		for($i=1; $i <= $pagination->total_pages(); $i++) {
			if($i == $page) {
				...
				echo " <a href=\"imports.php?page={$i}\" class=\"w3-bar-item w3-button \">{$i}</a> "; 
			}
		}
