...
<?php
	...
	$total_count = accounts_reciveable::count_all();
	...
	// for this page
	$sql = "SELECT * FROM accounts_reciveables ";
	$sql .= "LIMIT {$per_page} ";
	$sql .= "OFFSET {$pagination->offset()}";
	$accounts_reciveables = accounts_reciveable::find_by_sql($sql);
    $sql_all = "SELECT * FROM accounts_reciveables ";
    $all_accounts_reciveables = accounts_reciveable::find_by_sql($sql_all);;
...
<!--RFC: Only Change the text and create the accounts_reciveable.php and put it inside the actions -->
      <form class="w3-container" action="actions/add/accounts_reciveable.php" method="POST">
  ...
  <div id="id02" class="w3-modal">
    <div class="w3-modal-content w3-animate-zoom" style="max-width:100%">
      ...
      <div id="modal-data"  class="w3-col m12" style="">
        ...
        <!--RFC: This Foreach fill tha table with the graped data -->
            <?php foreach($all_accounts_reciveables as $emp): ?>
            <tr>
    ...
    <form action="accounts_reciveables.php" >
    ...
    <div class="w3-right w3-col m4">
        ...
        <select class="w3-select w3-border" style="float: left; width: 29%;" name="search_term">
            ...
            <!--RFC: you need to change the select tarm and the database name -->
                <?php
                   $sql = "SELECT name FROM accounts_reciveables;";
                   $result = accounts_reciveable::find_by_sql($sql);
                   //var_dump($result);
                   foreach($result as $op){
...
<br />
    ...
    <div class="w3-col m12" style="">
        ...
        ?>
            ...
            <?php foreach($accounts_reciveables as $emp): ?>
            <tr>
                <td><?php echo $emp->id; ?></td>
                <td><?php echo $emp->name; ?></td>
                <td><?php echo $emp->end_date; ?></td>
                ...
                <a href="edit_accounts_reciveable.php?id=<?php echo $emp->id?>" class="w3-button w3-button-small w3-teal" title="Edit"><i class="fa fa-pencil"></i></a>    
                <!--RFC: The delete button submit the id to the delete_accounts_reciveable.php -->
                <a href="actions/delete/delete_accounts_reciveable.php?id=<?php echo $emp->id?>" class="w3-button w3-button-small w3-red" title="Delete" onclick="return confirm('Are you sure you want to delete accounts_reciveable: <?php echo $emp->name?>?')"><i class="fa fa-trash"></i></a>
                <!--RFC: The print function grabs the data and open anothe page print_emplpyee.php from it you can print the table -->
                <a href="actions/print/print_accounts_reciveable.php?id=<?php echo $emp->id?>" class="w3-button w3-button-small w3-blue" title="Print"><i class="fa fa-print"></i></a></td>
            </tr>
            <?php endforeach; ?>
        </table>
        </div>
	...
	if($pagination->total_pages() > 1) {
    	...
    	echo "<a href=\"accounts_reciveables.php?page=";
      ...
      echo "\" class=\"w3-bar-item w3-button\">&laquo;</a> "; 
		...
		for($i=1; $i <= $pagination->total_pages(); $i++) {
			if($i == $page) {
				...
				echo " <a href=\"accounts_reciveables.php?page={$i}\" class=\"w3-bar-item w3-button \">{$i}</a> "; 
			}
		}
