...
<?php
	...
	$total_count = allowance::count_all();
	...
	$allowances = allowance::find_by_sql($sql);
    $sql_all = "SELECT * FROM allowances ";
    $all_allowances = allowance::find_by_sql($sql_all);;
...
<!--RFC: Only Change the text and create the allowance.php and put it inside the actions -->
      <form class="w3-container" action="actions/add/allowance.php" method="POST">
        <div class="w3-section">
          ...
          <?php
                ...
                ?></select>
          <input class="w3-input w3-border" type="date" placeholder="Allowence Date" name="allow_date" required>
      ...
      <div class="w3-container w3-border-top w3-padding-16 w3-light-grey">
        ...
        <div class="w3-col m7">
        <!--RFC: This button submits to allowance.php -->
  ...
  <div id="id02" class="w3-modal">
    <div class="w3-modal-content w3-animate-zoom" style="max-width:100%">
      ...
      <div id="modal-data"  class="w3-col m12" style="">
        ...
        <!--RFC: This Foreach fill tha table with the graped data -->
            <?php foreach($all_allowances as $emp): ?>
            <tr>
                <td><?php echo $emp->id; ?></td>
                <td><?php echo $emp->emp_id; ?></td>
                <td><?php echo $emp->allow_date; ?></td>
    ...
    <div class="w3-right w3-col m4">
        ...
        <select class="w3-select w3-border" style="float: left; width: 29%;" name="search_term">
            ...
            <option value="allow_date">Allow Date</option>
            ...
            <!--RFC: you need to change the select tarm and the database name -->
                <?php
                   $sql = "SELECT amount FROM allowances;";
                   $result = allowance::find_by_sql($sql);
                   //var_dump($result);
                   foreach($result as $op){
...
<br />
	...
	if($pagination->total_pages() > 1) {
    	...
    	echo "<a href=\"allowances.php?page=";
      ...
      echo "\" class=\"w3-bar-item w3-button\">&laquo;</a> "; 
		...
		for($i=1; $i <= $pagination->total_pages(); $i++) {
			if($i == $page) {
				...
				echo " <a href=\"allowances.php?page={$i}\" class=\"w3-bar-item w3-button \">{$i}</a> "; 
			}
		}
