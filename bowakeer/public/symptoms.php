...
<?php
	...
	$symptoms = symptom::find_by_sql($sql);
    $sql_all = "SELECT * FROM symptoms ";
    $all_symptoms = symptom::find_by_sql($sql_all);;
...
<header class="w3-container w3-theme" style="padding:21px 32px">
  ...
  <div class="w3-row">
    ...
    <div class="w3-bar">
   <a href="symptoms.php" class="w3-bar-item w3-button w3-round-medium">Home</a>
...
<!--RFC: Only Change the text and create the symptom.php and put it inside the actions -->
      <form class="w3-container" action="actions/add/symptom.php" method="POST">
        <div class="w3-section">
          <input class="w3-input w3-border" type="text" placeholder="Name" name="name" required>
          <input class="w3-input w3-border" type="number" placeholder="Cat Id" name="cat_id" required>
    ...
    <div class="w3-modal-content w3-animate-zoom" style="max-width:100%">
      ...
      <div id="modal-data"  class="w3-col m12" style="">
        ...
        <!--RFC: This Foreach fill tha table with the graped data -->
            <?php foreach($all_symptoms as $sym): ?>
            <tr>
                <td><?php echo $sym->id; ?></td>
                <td><?php echo $sym->name; ?></td>
				<td><?php echo $sym->cat_id; ?></td>
				<td><?php echo $sym->status; ?></td>
            </tr>
            <?php endforeach; ?>
        </table>
        </div>
    ...
    <div class="w3-col m12" style="">
        ...
        ?>
            ...
            <tr>
                ...
                <!--RFC: The edit button send the id to the edit_symptom.php -->
                <a href="edit_symptom.php?id=<?php echo $sym->id?>" class="w3-button w3-button-small w3-teal" title="Edit"><i class="fa fa-pencil"></i></a>    
                <!--RFC: The delete button submit the id to the delete_patient.php -->
                <a href="actions/delete/delete_symptom.php?id=<?php echo $sym->id?>" class="w3-button w3-button-small w3-red" title="Delete" onclick="return confirm('Are you sure you want to delete symptom: <?php echo $sym->name?>?')"><i class="fa fa-trash"></i></a>
                <!--RFC: The print function grabs the data and open anothe page print_emplpyee.php from it you can print the table -->
                <a href="actions/print/print_symptom.php?id=<?php echo $sym->id?>" class="w3-button w3-button-small w3-blue" title="Print"><i class="fa fa-print"></i></a></td>
            </tr>
            <?php endforeach; ?>
        </table>
        </div>
      ...
      echo "\" class=\"w3-bar-item w3-button\">&laquo;</a> "; 
		...
		for($i=1; $i <= $pagination->total_pages(); $i++) {
			if($i == $page) {
				...
				echo " <a href=\"symptoms.php?page={$i}\" class=\"w3-bar-item w3-button \">{$i}</a> "; 
			}
		}
